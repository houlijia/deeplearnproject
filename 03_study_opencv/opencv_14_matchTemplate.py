...
lena_img = cv2.imread('00.image/lena512color.tiff', 0)
lena_img = cv2.cvtColor(lena_img, cv2.COLOR_BGR2RGB)
lena_face = cv2.imread('./00.image/lena_face.jpg', 0)
...
plt.subplot(121), plt.title('template image'), plt.imshow(lena_face), plt.axis('off')
plt.subplot(122), plt.title('target image'), plt.axis('off'), plt.imshow(processed_image)
plt.show()
